...
class TruthOrDareGame {
    ...
    showCreateRoom() {
        ...
        const playerNameInput = document.getElementById('hostName');

        // Enable start button when name is typed
        playerNameInput.addEventListener('input', (e) => {
            this.updateHostName(e.target.value);

            // Auto-generate room if name exists and roomCode is empty
            if (e.target.value.trim().length > 0 && !this.roomCode) {
                this.createRoom(e.target.value.trim());
            }
        });
    }
    ...
    createRoom(playerName) {
        ...
        // Show a temporary room code immediately
        if (!this.roomCode) {
            const tempCode = Math.random().toString(36).substring(2, 8).toUpperCase();
            this.roomCode = tempCode;
            const roomCodeElems = document.querySelectorAll('#currentRoomCode, #roomCode');
            roomCodeElems.forEach(elem => elem.textContent = tempCode);
        }
    }
    
    // ========================
    // Start game now only triggers actual start
    // ========================
    startGame() {
        if (!this.playerName.trim()) {
            alert('Please enter your name');
    ...
    // ... rest of your original methods remain unchanged (updatePlayersList, updateMiniPlayerList, updateGameDisplay, selectTruth, selectDare, showCard, resetCard, nextPlayer, isCurrentPlayer, toggleVoice, toggleChat, sendMessage, addChatMessage, updateVoiceStatus, toggleMute, loadQuestions, showQuestionModal, hideQuestionModal, switchTab, addQuestion, deleteQuestion, updateQuestionsDisplay, updateQuestionsList, resetGame)
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Auto-focus name input on create room page
...
// Add keyboard shortcuts for game room
document.addEventListener('keydown', (e) => {
    if (window.game && window.game.currentPage === 'gameRoomPage') {
        if (e.key === 't' || e.key === 'T') {
            document.getElementById('truthBtn').click();
